...
class ReceiveReportView(APIView):
    ...
    """
    @extend_schema(
        request=ReceiveReportSerializer,
        responses={
            (200, "application/json"): {
                "type": "object",
    ...
    def post(self, request):
        ...
        :returns Response object with
            ...
            message and corresponding status if error
        """
        serializer = ReceiveReportSerializer(data=request.data)
        if not serializer.is_valid():
            log_error(request.path, request.data, status.HTTP_400_BAD_REQUEST, serializer.errors)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class GetReportView(APIView):
    ...
    @extend_schema(
        parameters=[
          OpenApiParameter("report_id", OpenApiTypes.INT, OpenApiParameter.PATH), # path variable was overridden
        ],
        responses=(
...
class GetUserReportsView(APIView):
    ...
    @jwt_auth_required
    @extend_schema(
        responses=ServiceRequestSerializer,
        auth=[ {"bearerAuth": [] }]
    )
    def get(self, request, user=None):
        """
